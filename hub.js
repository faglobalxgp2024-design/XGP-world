/* HUN.JS - LEGO PREMIUM (single-file) v2.5
 * 개선 반영:
 * 1) 배경/배치 퀄리티: 도로/존 재배치(겹침 제거), 오브젝트 배치(간격/구역 회피/정돈)
 * 2) 캐릭터 퀄리티: 미니피규어 쉐이딩/프린트/실루엣 개선
 * 3) 모바일 조작: D-Pad → 아날로그 조이스틱(휠)로 변경(대각선 자유)
 * 4) 경계/표식: 도로/횡단보도/신호등이 존 안으로 들어가지 않도록 생성/필터링, 존 경계 시각 강화
 *
...
(() => {
  ...
  function ensureUI() {
    ...
    joy.style.width = "142px";
    joy.style.height = "142px";
    ...
    joyRing.style.width = "114px";
    joyRing.style.height = "114px";
    ...
    joyKnob.style.width = "62px";
    joyKnob.style.height = "62px";
    ...
    function setJoy(ax, ay) {
      ...
      joyBase.style.background = joyState.active ? "rgba(255,255,255,0.84)" : "rgba(255,255,255,0.72)";
    }
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    if (SPRITE_SRC && USE_SPRITE_IF_LOADED) {
      ...
      im.onload = () => {
        sprite.img = im; sprite.loaded = true;
        sprite.w = im.naturalWidth || 1; sprite.h = im.naturalHeight || 1;
      };
    ...
    function seedCars(rng) {
      ...
      const makeCar = (r, axis) => {
        ...
        const speed = 96 + rng() * 112;
        if (axis === "h") {
          const lane = rng() < 0.5 ? 0 : 1;
          const dir = rng() < 0.5 ? 1 : -1;
          return { kind:"car", axis:"h", dir, color:col, speed,
            w: 56 + rng()*20, h: 26 + rng()*8,
            x: r.x + rng()*r.w,
          ...
          const lane = rng() < 0.5 ? 0 : 1;
          const dir = rng() < 0.5 ? 1 : -1;
          return { kind:"car", axis:"v", dir, color:col, speed,
            w: 26 + rng()*8, h: 60 + rng()*20,
      ...
      for (const r of roads) {
        ...
        if (rectInAnyZone(r, 10)) continue;
    ...
    function seedProps(rng) {
      ...
      function okLamp(x,y){
        ...
        for(const r of roads){
          ...
          if(Math.hypot(nx-x, ny-y) < 90){ near = true; break; }
        }
        return near;
      }
      const lampPts = scatterPoints(rng, 28, 120, 9000, okLamp);
      for (const p of lampPts) props.push({ kind:"lamp", x:p.x, y:p.y, s: 0.9 + rng()*0.55 });

      // 3) benches near sidewalks
      ...
      const benchPts = scatterPoints(rng, 14, 160, 7000, okBench);
    ...
    function buildPatterns(rng){
      grassPattern = makePattern(520,520,(g,w,h)=>{
        g.fillStyle="#39d975"; g.fillRect(0,0,w,h);
        // subtle grid + noise
        g.globalAlpha=0.045;
      ...
      roadPattern = makePattern(260,260,(g,w,h)=>{
        g.fillStyle="#262c37"; g.fillRect(0,0,w,h);
        // fine grain
        g.globalAlpha=0.16;
      ...
      sidewalkPattern = makePattern(240,240,(g,w,h)=>{
        g.fillStyle="#f5efe7"; g.fillRect(0,0,w,h);
        g.globalAlpha=0.12;
      ...
      brickPattern = makePattern(360,360,(g,w,h)=>{
        g.fillStyle="#d9c6a3"; g.fillRect(0,0,w,h);
        g.globalAlpha=0.32;
    ...
    function layoutRoadNetwork() {
      ...
      // main lanes (zones won't overlap these)
      const left = WORLD.w*0.08, right = WORLD.w*0.92;
      const top = WORLD.h*0.10, bot = WORLD.h*0.92;
      ...
      addRoadH(yMid, left, right, 132);
      addRoadH(yLow, left+120, right-120, 120);

      addRoadV(xMid, top, bot, 124);
      addRoadV(xL,  top, yMid+160, 118);
      addRoadV(xR,  top, yMid+160, 118);
      ...
      // crossings at intersections (only where not inside zones)
    ...
    function layoutWorld(rng){
      ...
      // zones placed to avoid road overlap
      ZONES.game = {
        x: WORLD.w*0.08,
        y: WORLD.h*0.14,
        w: WORLD.w*0.36,
      ...
      for(const p of portals){
        ...
        if(p.key==="avoid"||p.key==="archery"||p.key==="janggi"||p.key==="omok"||p.key==="snow"||p.key==="jump"){
          clampIntoZone(p, ZONES.game, d);
        } else if(p.key==="twitter"||p.key==="telegram"||p.key==="wallet"||p.key==="market"||p.key==="support"){
          clampIntoZone(p, ZONES.community, d);
        } else {
          clampIntoZone(p, ZONES.ads, d);
        }
      }
    ...
    function blockSpan(html, opt={}){
      ...
      return `<span style="
        ...
        filter:none; backdrop-filter:none; -webkit-backdrop-filter:none; text-shadow:none;
      ">${html}</span>`;
    }
    ...
    function openPortalUI(p){
      ...
      modalState.portal = p;
      openModal(`🧱 ${p.label}`, `입장하시겠습니까?<br/><span style="opacity:.95;font-size:22px;font-weight:1200;">Enter / E</span>`,
    ...
    function drawRoadsAndSidewalks(){
      for(const r of roads){
        ...
        // dashed center line (only on roads, never on zones now)
        ctx.globalAlpha=0.42;
      ...
      for(const s of sidewalks){
        // sidewalks can overlap zones edge, but looks okay; still avoid deep overlap
    ...
    function drawZonesWorld(t){
      function drawZone(z){
        ...
        roundRect(z.x + z.w/2 - 72, z.y + z.h - 16, 144, 8, 4);
        ctx.fill();
        ctx.globalAlpha=1;
    ...
    function drawSignal(s, t){
      ...
      roundRect(-4,-18,8,48,6); ctx.fill();
      ctx.fillStyle="rgba(10,14,24,0.92)";
      roundRect(-14,-42,28,28,10); ctx.fill();

      const dot = (yy, on, col)=>{
        ctx.save();
        ctx.globalAlpha = on ? 1 : 0.25;
        ctx.fillStyle=col;
        ctx.beginPath(); ctx.arc(0,yy,4.6,0,Math.PI*2); ctx.fill();
        if(on){
          ctx.globalAlpha=0.18;
          ctx.beginPath(); ctx.arc(0,yy,10,0,Math.PI*2); ctx.fill();
        }
        ctx.restore();
      };
      dot(-34, redOn, "#ff3b30");
    ...
    function legoStyleForType(type){
      ...
      const accentBy={
        ...
        wallet:"#34c759", market:"#ffcc00", support:"#8b5cf6",
        bbq:"#ff2d55", baskin:"#ff66cc", paris:"#0a84ff"
      };
    ...
    function drawLegoStudRow(x,y,w,count,color){
      ...
      for(let i=0;i<count;i++){
        ...
        ctx.globalAlpha=0.18;
        ctx.fillStyle="rgba(255,255,255,0.92)";
        ctx.beginPath(); ctx.arc(cx-1.6,y-1.8,r*0.55,0,Math.PI*2); ctx.fill();
        ctx.globalAlpha=1;
        ctx.fillStyle=color;
      }
      ctx.restore();
    ...
    function drawLegoBrickGrid(x,y,w,h){
      ...
      for(let yy=y; yy<y+h; yy+=rowH){
        ...
        for(let xx=x+off; xx<x+w; xx+=colW){
          ctx.beginPath(); ctx.moveTo(xx,yy+2); ctx.lineTo(xx,Math.min(yy+rowH-2,y+h-2)); ctx.stroke();
        }
        alt=!alt;
      }
      ctx.restore();
    ...
    function drawLegoWindow(x,y,w,h,frameCol,glassA,glassB){
      ...
      roundRect(x-2,y-2,w+4,h+4,14); ctx.fill();
    ...
    function drawLegoDoor(x,y,w,h,doorCol,frameCol,knobCol){
      ...
      roundRect(x+w*0.72,y+h*0.46,7,12,5); ctx.fill();
      ctx.restore();
    }

    function drawPortalBuilding(p, t){
      const S = legoStyleForType(p.type);
      // key-based sign overrides
      ...
      // base plate + grass strip
      const baseH=18;
      ctx.save();
      ctx.fillStyle="rgba(120,94,255,0.52)";
      roundRect(p.x+6, p.y+p.h-baseH+6, p.w-12, baseH, 12); ctx.fill();
      ctx.fillStyle=S.grass;
      roundRect(p.x+10, p.y+p.h-baseH+2, p.w-20, 14, 10); ctx.fill();
      ctx.restore();
      ...
      ctx.lineWidth=2;
      roundRect(bodyX, bodyY, bodyW, bodyH, 18); ctx.fill(); ctx.stroke();

      // cap
      ctx.fillStyle = shade(S.wall, +12);
      roundRect(bodyX, bodyY-14, bodyW, 22, 18); ctx.fill();
      ctx.globalAlpha=0.14;
      ctx.fillStyle="rgba(255,255,255,0.9)";
      roundRect(bodyX+10, bodyY-10, bodyW-20, 8, 10); ctx.fill();
      ctx.globalAlpha=1;
      ...
      roundRect(bodyX+6, bodyY-22, 34, 18, 6); ctx.fill();
      roundRect(bodyX+bodyW-40, bodyY-22, 34, 18, 6); ctx.fill();
      ctx.restore();
      ...
      if(p.status!=="open" || !p.url){
        ...
        roundRect(bx,by,122,28,14); ctx.fill();
        ctx.fillStyle="rgba(255,255,255,0.98)";
    ...
    function drawTree(o){
      ...
      roundRect(x-10*s, y-22*s, 20*s, 48*s, 10*s); ctx.fill();
      ...
      ctx.beginPath(); ctx.ellipse(x+8*s,y-44*s,30*s,22*s,0,0,Math.PI*2); ctx.fill();
      ctx.globalAlpha=1;

      // highlight
      ctx.globalAlpha=0.10;
      ctx.fillStyle="rgba(255,255,255,0.92)";
      ctx.beginPath(); ctx.ellipse(x-6*s,y-66*s,18*s,12*s,0,0,Math.PI*2); ctx.fill();
      ctx.globalAlpha=1;
      ctx.restore();
    }
    ...
    function drawLamp(o,t){
      ...
      roundRect(x-5*s,y-42*s,10*s,70*s,8*s); ctx.fill();
      ctx.fillStyle="#ffffff";
      roundRect(x-16*s,y-54*s,32*s,22*s,10*s); ctx.fill();
      glossyHighlight(x-16*s,y-54*s,32*s,22*s,0.18);

      ctx.globalAlpha=0.06+0.18*pulse;
      ctx.fillStyle="#ffd66b";
      ctx.beginPath(); ctx.ellipse(x,y-10*s,34*s,54*s,0,0,Math.PI*2); ctx.fill();
      ctx.restore();
    }
    ...
    function drawBench(o){
      ...
      roundRect(x-42*s,y-2*s,84*s,18*s,10*s); ctx.fill();
    ...
    function drawFlower(o,t){
      ...
      ctx.lineWidth=4*s;
      ctx.lineCap="round";
      ctx.beginPath(); ctx.moveTo(x,y+8*s); ctx.lineTo(x+wig,y-12*s); ctx.stroke();
      ...
      ctx.fillStyle="#ffffff";
      ctx.beginPath(); ctx.arc(x+wig,y-16*s,3.6*s,0,Math.PI*2); ctx.fill();
      ctx.restore();
    }
    ...
    function drawSign(s){
      ...
      ctx.lineWidth=2;
      roundRect(-72,-62,144,44,18); ctx.fill(); ctx.stroke();
      ctx.fillStyle="rgba(10,14,24,0.92)";
    ...
    function drawEmblem(e){
      ...
      ctx.strokeStyle="rgba(0,0,0,0.12)";
      ctx.lineWidth=2;
      roundRect(-20,-6,40,18,10); ctx.fill(); ctx.stroke();
      ctx.fillStyle=S.accent;
      ctx.beginPath(); ctx.arc(0,-18,6.5,0,Math.PI*2); ctx.fill();
      ctx.restore();
    }
    ...
    function drawCar(c){
      ...
      if(c.axis==="h"){
        ...
        ctx.fillStyle=base;
        roundRect(-w*0.52,-h*0.4,w*1.04,h*0.8,12); ctx.fill();
        glossyHighlight(-w*0.52,-h*0.4,w*1.04,h*0.8,0.10);
        ctx.fillStyle=shade(base,+16);
        roundRect(-w*0.2,-h*0.58,w*0.4,h*0.28,10); ctx.fill();
        ...
        ctx.fillStyle=g;
        roundRect(-w*0.18,-h*0.34,w*0.36,h*0.26,8); ctx.fill();

        ctx.fillStyle="rgba(10,14,24,0.16)";
        roundRect(-w*0.54,h*0.14,w*1.08,h*0.18,10); ctx.fill();
      ...
      ctx.fillStyle=base;
      roundRect(-w*0.55,-h*0.5,w*1.1,h*1.0,12); ctx.fill();
      glossyHighlight(-w*0.55,-h*0.5,w*1.1,h*1.0,0.10);
      ctx.fillStyle=shade(base,+16);
      roundRect(-w*0.34,-h*0.62,w*0.68,h*0.26,10); ctx.fill();
      ...
      ctx.fillStyle=gg;
      roundRect(-w*0.32,-h*0.3,w*0.64,h*0.52,10); ctx.fill();

      ctx.fillStyle="rgba(10,14,24,0.16)";
      roundRect(-w*0.56,h*0.32,w*1.12,h*0.16,10); ctx.fill();
    ...
    function drawSpriteCharacter(x,y){
      ...
      ctx.fillStyle="rgba(10,14,24,0.42)";
      ctx.beginPath(); ctx.ellipse(x,y+28,22,8,0,0,Math.PI*2); ctx.fill();
      ctx.restore();
    ...
    function drawCapOnHead(hatColor){
      ...
      roundRect(-14,-20,28,15,9); ctx.fill();
      ctx.globalAlpha=0.14;
      ctx.fillStyle="rgba(255,255,255,0.95)";
      roundRect(-10,-17,20,5,6); ctx.fill();
      ctx.globalAlpha=1;
      ...
      roundRect(-12,-6,24,4,3); ctx.fill();
      ctx.globalAlpha=1;
      ctx.restore();
    }

    // improved minifig: better silhouette + torso print + shoes + shading
    function drawMinifig(x,y,opts=null){
      ...
      ctx.globalAlpha=0.24;
      ctx.fillStyle="rgba(10,14,24,0.42)";
      ctx.beginPath(); ctx.ellipse(x,y+28,20,7,0,0,Math.PI*2); ctx.fill();
      ctx.restore();
      ...
      if(dir==="down"){
        ctx.beginPath(); ctx.arc(-5,-22,2.2,0,Math.PI*2); ctx.arc(5,-22,2.2,0,Math.PI*2); ctx.fill();
        ctx.strokeStyle="rgba(10,14,24,0.62)"; ctx.lineWidth=2;
        ctx.beginPath(); ctx.arc(0,-18,6,0,Math.PI); ctx.stroke();
      } else if(dir==="up"){
        ctx.globalAlpha=0.22;
        ...
        ctx.beginPath(); ctx.arc(7,-22,2.1,0,Math.PI*2); ctx.fill();
        ctx.strokeStyle="rgba(10,14,24,0.62)"; ctx.lineWidth=2;
        ctx.beginPath(); ctx.arc(9,-18,6,-0.25,Math.PI-0.55); ctx.stroke();
      }
      ctx.restore();
      ...
      if(!side){
        ...
        ctx.fillStyle="rgba(255,255,255,0.92)";
        roundRect(-9,6,18,6,6); ctx.fill();
        roundRect(-9,15,18,4,6); ctx.fill();
        ctx.globalAlpha=1;
        ctx.restore();

        // arms
        ctx.fillStyle=torso; roundRect(-22,2,10,18,8); ctx.fill();
        ...
        // side torso slimmer
        ctx.fillStyle=torso;
        roundRect(-9,-4,18,28,12); ctx.fill();
        ...
        roundRect(-16,4,8,14,8); ctx.fill();
        ctx.restore();

        // front arm
        ctx.fillStyle=torso;
        roundRect(9,3,10,18,8); ctx.fill();
        ctx.fillStyle=skin;
        roundRect(9,15+armSwing,10,8,6); ctx.fill();

        // ONE leg
        ctx.fillStyle=pants;
        roundRect(2,22,12,16,6); ctx.fill();

        // shoe
        ctx.fillStyle="rgba(10,14,24,0.78)";
        ctx.beginPath(); ctx.ellipse(9,40+legSwing,6.2,3.0,0,0,Math.PI*2); ctx.fill();
      }
    ...
    function drawNPC(key,x,y){
      ...
      if(theme.acc==="bow"){
        ...
        ctx.lineWidth=3;
        ctx.beginPath(); ctx.arc(22,12,12,-1.2,1.2); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(12,12); ctx.lineTo(32,12); ctx.stroke();
      } else if(theme.acc==="stone"){
        ctx.fillStyle="rgba(10,14,24,0.20)";
        roundRect(16,18,14,10,5); ctx.fill();
        ctx.fillStyle="#ffffff";
        roundRect(18,20,10,6,4); ctx.fill();
    ...
    function drawRoamer(n,palette){
      ...
      drawMinifig(n.x,n.y,{
        moving:true, dir:n.dir,
        animT:n.t, bobT:n.t*0.9,
        torso:c.torso, pants:c.pants, hat:c.hat
      });
    }
    ...
    function drawWorldTitle(){
      ...
      ctx.strokeStyle="rgba(0,0,0,0.10)";
      ctx.lineWidth=2;
      roundRect(x,y,bw,bh,18); ctx.fill(); ctx.stroke();
    ...
    function drawMiniMap(){
      ...
      ctx.strokeStyle="rgba(0,0,0,0.10)";
      ctx.lineWidth=2;
      roundRect(x,y,mw,mh,18); ctx.fill(); ctx.stroke();
      ctx.globalAlpha=0.10;
      ctx.fillStyle="rgba(10,14,24,0.85)";
      roundRect(x+10,y+mh-16,mw-20,8,8); ctx.fill();
      ctx.globalAlpha=1;

      const ix=x+10, iy=y+10, iw=mw-20, ih=mh-20;
      ctx.save();
      roundRect(ix,iy,iw,ih,14); ctx.clip();
      ...
      for(const r of roads){
        roundRect(mx(r.x),my(r.y),r.w*s,r.h*s,8); ctx.fill();
      }

      // zones (tiny)
      ctx.globalAlpha=0.28;
      ctx.fillStyle="#0a84ff";
      roundRect(mx(ZONES.game.x),my(ZONES.game.y),ZONES.game.w*s,ZONES.game.h*s,8); ctx.fill();
      ctx.fillStyle="#34c759";
      roundRect(mx(ZONES.community.x),my(ZONES.community.y),ZONES.community.w*s,ZONES.community.h*s,8); ctx.fill();
      ctx.fillStyle="#ff2d55";
      roundRect(mx(ZONES.ads.x),my(ZONES.ads.y),ZONES.ads.w*s,ZONES.ads.h*s,8); ctx.fill();
      ctx.globalAlpha=1;
    ...
    function update(dt, t, rng){
      ...
      // mobile auto open modal only when entering portal zone changes
    ...
    function loop(now){
      ...
      // deterministic-ish RNG per frame
    ...
    canvas.addEventListener("pointerdown",(e)=>{
      ...
      // mobile double tap to enter while modal open
